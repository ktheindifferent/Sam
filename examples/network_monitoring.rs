...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(interfaces) = monitor.read_network_stats().await {
        for (name, interface) in interfaces.iter() {
            ...
            println!("  Errors: RX={}, TX={}", interface.rx_errors, interface.tx_errors);
            println!();
        }
    }
    ...
    if let Ok(speeds) = monitor.calculate_speeds().await {
        for (interface, speed) in speeds.iter() {
            ...
            println!("  Download: {:.2} Mbps ({:.0} KB/s)", 
                     speed.download_speed_mbps, 
                     speed.download_speed_bps / 1024.0);
            println!("  Upload: {:.2} Mbps ({:.0} KB/s)", 
    ...
    if let Ok(metrics) = monitor.get_metrics().await {
        println!("Total Download Speed: {:.2} Mbps", metrics.total_download_mbps);
    ...
    println!("  Latency Check Interval: {} ms", config.latency_check_interval_ms);
    println!("  High Latency Threshold: {} ms", config.alert_thresholds.high_latency_ms);
    println!();
    
    // Start monitoring (runs in background)
    let monitor_configured = NetworkMonitor::from_config(config.clone());
    monitor_configured.start_monitoring_with_config(config).await;
    ...
    if let Ok(final_metrics) = monitor_configured.get_metrics().await {
        ...
        println!("--------------------------------");
        println!("Total Download: {:.2} Mbps", final_metrics.total_download_mbps);
