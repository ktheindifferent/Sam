...
fn bench_base64_encoding(c: &mut Criterion) {
    ...
    let test_credentials = vec![
        ...
        ("very_long_client_id_for_testing_purposes", "very_long_client_secret_for_testing_purposes"),
    ];
    ...
    for (id, secret) in test_credentials {
        ...
        group.bench_with_input(
            ...
            |b, &(id, secret)| {
                b.iter(|| {
                    STANDARD.encode(format!("{}:{}", black_box(id), black_box(secret)))
                });
            },
        );
    }
...
fn bench_file_path_operations(c: &mut Criterion) {
    ...
    c.bench_function("path_string_format", |b| {
        b.iter(|| {
            let _ = format!("/opt/sam/tmp/youtube/downloads/{}.mp4", black_box("test_video"));
        });
    });
}
