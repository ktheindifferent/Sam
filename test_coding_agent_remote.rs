...
async fn main() {
    ...
    let agent = sam::services::coding::CodingAgentService::new_with_defaults().await;
    ...
    match agent.generate_response(
        ...
        Err(e) => {
            println!("\n❌ Error: {}", e);
            println!("\nTroubleshooting:");
            println!("1. Check if the remote server is running: curl http://172.16.0.125:11434/api/tags");
            println!("2. Verify the configuration file: cat ~/.sam/coding_agent/ollama_config.json");
