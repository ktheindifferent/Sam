...
fn main() {
        ...
        Ok(token) => {
            ...
            match decode::<JwtClaims>(&token, &decode_key, &validation) {
                Ok(token_data) => {
                    ...
                    println!("   User: {}, Client: {}", token_data.claims.sub, token_data.claims.client_id);
    ...
    for malformed in malformed_tokens {
        ...
        match decode::<JwtClaims>(malformed, &decode_key, &validation) {
            Ok(_) => println!("   ✗ Malformed token '{}' was incorrectly accepted", malformed),
            Err(_) => println!("   ✓ Malformed token correctly rejected: '{}'", 
                if malformed.is_empty() { "(empty)" } else { malformed }),
        }
    }
