...
fn test_binary_exists_and_executable() {
    ...
    if std::path::Path::new(debug_binary).exists() {
        ...
        let info_output = Command::new(debug_binary).arg("--help").output();
...
mod config_tests {
    ...
    fn test_project_structure() {
        ...
        assert!(
            std::path::Path::new("src/lib/mod.rs").exists(),
            "lib/mod.rs should exist"
        );
        assert!(
            std::path::Path::new("src/sam.rs").exists(),
            "sam.rs should exist"
        );
    }
    ...
    fn test_cargo_toml_valid() {
        ...
        assert!(
            cargo_content.contains("name = \"sam\""),
...
mod integration_tests {
    ...
    fn test_compilation_time() {
        ...
        assert!(
            ...
            "Compilation should not take more than 5 minutes"
        );
