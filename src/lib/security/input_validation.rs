...
static XSS_PATTERNS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)(<script|javascript:|onerror=|onload=|onclick=|<iframe|<embed|<object)")
        .unwrap_or_else(|_| Regex::new(r".*").unwrap())
});

static PATH_TRAVERSAL_PATTERNS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(\.\./|\.\.\%2[fF]|\.\.\\|\.\.\%5[cC])")
        .unwrap_or_else(|_| Regex::new(r".*").unwrap())
});
...
pub fn validate_username(username: &str) -> Result<String, String> {
    let username_regex =
        Regex::new(r"^[a-zA-Z0-9_]{3,32}$").unwrap_or_else(|_| Regex::new(r".*").unwrap());
