...
impl File {
    ...
    pub fn save(&self) -> Result<&Self> {
        ...
        if let Some(folder_tree) = self.file_folder_tree.clone() {
            client.execute(
                "UPDATE storage_file SET file_folder_tree = $1, updated_at = $2 WHERE oid = $3;",
                &[&folder_tree.join("/"), &self.updated_at, &ads.oid],
            )?;
        }

        if let Some(file_data) = self.file_data.clone() {
            client.execute(
                "UPDATE storage_file SET file_data = $1, updated_at = $2 WHERE oid = $3;",
                &[&file_data, &self.updated_at, &ads.oid],
            )?;
        }
    ...
    pub async fn select_async(
        ...
        let jsons = Config::pg_select_async(
            Self::sql_table_name(),
            None,
            limit,
            offset,
    ...
    pub async fn select_lite_async(
        limit: Option<usize>,
        offset: Option<usize>,
        order: Option<String>,
        query: Option<PostgresQueries>,
    ...
    pub fn destroy(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row(oid, "storage_file".to_string())
    }

    /// Asynchronously deletes a File from the database by OID.
    pub async fn destroy_async(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row_async(oid, "storage_file".to_string()).await
    }
        ...
        for file in files_without_data {
            if !Path::new(file.path_on_disk().as_str()).exists() {
                if file.storage_location_oid == *"SQL" {
                    ...
                    pg_query
                        .queries
                        .push(crate::memory::PGCol::String(file.oid.clone()));
                    pg_query.query_columns.push("oid =".to_string());
