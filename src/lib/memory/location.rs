...
impl Location {
    ...
    pub fn destroy(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row(oid, "locations".to_string())
    }
    ...
    pub async fn count_async() -> Result<i64> {
        ...
        let execquery = format!("SELECT COUNT(*) FROM {}", Self::sql_table_name()); // Safe: returns hardcoded "locations"
    ...
    pub async fn destroy_async(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row_async(oid, "locations".to_string()).await
    }
}
