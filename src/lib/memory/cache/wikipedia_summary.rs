...
impl WikipediaSummary {
    ...
    pub fn save(object: Self) -> Result<Self> {
        ...
        pg_query
            .queries
            .push(crate::memory::PGCol::String(object.oid.clone()));
        ...
        if rows.is_empty() {
            ...
            client.execute("UPDATE cache_wikipedia_summaries SET topics = $1, summary = $2, timestamp = $3 WHERE oid = $4;", 
                ...
                &ads.oid])?;
            let statement_two =
                client.prepare("SELECT * FROM cache_wikipedia_summaries WHERE oid = $1")?;
    ...
    pub fn destroy(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row(oid, "cache_wikipedia_summaries".to_string())
    }
    ...
    pub async fn save_async(object: Self) -> Result<Self> {
        ...
        if rows.is_empty() {
            ...
            let statement_two = client
                .prepare("SELECT * FROM cache_wikipedia_summaries WHERE oid = $1")
                .await?;
