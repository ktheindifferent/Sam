...
use crate::memory::PostgresQueries;
...
impl Notification {
    ...
    pub fn save(&self) -> Result<Self> {
        ...
        if rows.is_empty() {
            ...
            client.execute(
                "UPDATE human_notifications SET message = $1, seen = $2 WHERE oid = $3;",
                &[&self.message, &self.seen, &ads.oid],
            )?;

            let statement_two =
                client.prepare("SELECT * FROM human_notifications WHERE oid = $1")?;
            let rows_two = client.query(&statement_two, &[&self.oid])?;
    ...
    pub fn select(
        ...
        let jsons = crate::memory::Config::pg_select(
            Self::sql_table_name(),
            None,
            limit,
            offset,
    ...
    pub fn destroy(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row(oid, "human_notifications".to_string())
    }
    ...
    pub async fn save_async(&self) -> Result<Self> {
        ...
        if rows.is_empty() {
            ...
            client
                ...
                .await?;
            let statement_two = client
                .prepare("SELECT * FROM human_notifications WHERE oid = $1")
                .await?;
    ...
    pub async fn destroy_async(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row_async(oid, "human_notifications".to_string()).await
    }
}
