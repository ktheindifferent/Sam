...
impl Thing {
    ...
    pub fn destroy(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row(oid, "things".to_string())
    }
    ...
    pub async fn destroy_async(oid: String) -> Result<bool> {
        crate::memory::Config::destroy_row_async(oid, "things".to_string()).await
    }
    ...
    pub fn load_by_id(thing_id: &str) -> Result<Self> {
        ...
        pg_query
            .queries
            .push(crate::memory::PGCol::String(thing_id.to_string()));
        pg_query.query_columns.push("oid =".to_string());
