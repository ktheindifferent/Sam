    ...
    pub fn connection_string(&self) -> String {
        match self {
            DatabaseEngine::SQLite => {
                let path =
                    std::env::var("SQLITE_PATH").unwrap_or_else(|_| "./data/sam.db".to_string());
                format!("sqlite://{}", path)
            }
            DatabaseEngine::PostgreSQL => {
                ...
                let pass = std::env::var("POSTGRES_PASSWORD")
                    .unwrap_or_else(|_| "sampassword".to_string());
            ...
            DatabaseEngine::MySQL | DatabaseEngine::MariaDB => {
                ...
                let pass =
                    std::env::var("MYSQL_PASSWORD").unwrap_or_else(|_| "sampassword".to_string());
...
pub trait DatabaseConnection: Send + Sync {
    ...
    where
        F: FnOnce(Box<dyn DatabaseTransaction + '_>) -> futures::future::BoxFuture<'_, Result<R>>
            + Send,
        R: Send;
        ...
        let connection = match engine {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "Database engine {:?} not yet implemented",
                    engine
                ));
            }
        };
...
impl SqliteConnection {
    pub async fn new(connection_string: &str) -> Result<Self> {
        ...
        let conn = RusqliteConnection::open(path).context("Failed to open SQLite database")?;
