...
impl MigrationRunner {
    ...
    pub async fn run(&self) -> Result<()> {
        info!(
            "Starting migration runner (dry_run: {}, auto_backup: {})",
        ...
        let pending_migrations: Vec<&Box<dyn Migration>> = self
            .migrations
            .iter()
    ...
    pub async fn rollback(&self, target_version: Option<i64>) -> Result<()> {
        ...
        let target = target_version
            .unwrap_or_else(|| applied.iter().map(|m| m.version).max().unwrap_or(0) - 1);

        if self.auto_backup && !self.dry_run {
            self.create_backup().await?;
        }

        let migrations_to_rollback: Vec<AppliedMigration> =
            applied.into_iter().filter(|m| m.version > target).collect();
        ...
        for applied_migration in migrations_to_rollback.iter().rev() {
                ...
                .find(|m| m.version() == applied_migration.version)
            {
                ...
                warn!(
                    "Migration {} not found in codebase, skipping rollback",
    ...
    async fn rollback_migration(&self, migration: &dyn Migration) -> Result<()> {
        ...
        migration.down(&tx).await.context(format!(
        ...
        info!(
            "Migration {} rolled back successfully in {}ms",
            migration.version(),
