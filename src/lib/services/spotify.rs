...
static PLAYBACK_THREAD_ID: Lazy<Mutex<Option<String>>> = Lazy::new(|| Mutex::new(None));
...
fn acquire_state_lock() -> Result<MutexGuard<'static, SpotifyService>, String> {
    SPOTIFY_STATE
        ...
        .or_else(
            |e| -> Result<
                MutexGuard<'static, SpotifyService>,
                std::sync::PoisonError<MutexGuard<'static, SpotifyService>>,
            > {
...
fn acquire_thread_lock() -> Result<MutexGuard<'static, Option<String>>, String> {
    PLAYBACK_THREAD_ID
        ...
        .or_else(
            |e| -> Result<
                MutexGuard<'static, Option<String>>,
                std::sync::PoisonError<MutexGuard<'static, Option<String>>>,
            > {
                // Attempt to recover from poisoned mutex
...
#[cfg(feature = "timeout")]
fn try_acquire_state_lock_timeout(
    timeout: Duration,
) -> Result<MutexGuard<'static, SpotifyService>, String> {
        ...
        let thread_id =
            thread_manager::spawn_with_config(config, move |shutdown_signal, _health_rx| {
                    ...
                    && !SHUTDOWN_SIGNAL.load(Ordering::Relaxed)
                {
                    {
                        let s =
                            match state_arc.lock().or_else(
                                |e: std::sync::PoisonError<
                                    std::sync::MutexGuard<SpotifyService>,
                                >|
                                 -> Result<
                                    std::sync::MutexGuard<SpotifyService>,
                                    std::sync::PoisonError<std::sync::MutexGuard<SpotifyService>>,
                                > {
                                    log::warn!(
                                        "Recovering from poisoned mutex in playback thread: {}",
                                        e
                                    );
                                    Ok(e.into_inner())
                                },
                                ...
                                Err(_) => {
                                    log::error!(
                                        "Critical: Failed to acquire state lock in playback thread"
                                    );
                                    break;
                                }
                            };
                        match s.status {
                            SpotifyStatus::Playing => {
                                if has_credentials {
                                    ...
                                    info!(
                                        "[Spotify] Playing music... (shuffle: {}) [SIMULATED]",
                                        s.shuffle
                                    );
                                }
                            }
...
mod tests {
    ...
    fn test_spotify_status_initial() {
        ...
        assert!(matches!(
            state.status,
            SpotifyStatus::Stopped | SpotifyStatus::Playing | SpotifyStatus::Paused
        ));
    }
    ...
    async fn test_spotify_api_authenticate() {
        ...
        Mock::given(method("POST"))
            ...
            .and(header(
                "Authorization",
                "Basic dGVzdF9pZDp0ZXN0X3NlY3JldA==",
            ))
    ...
    async fn test_spotify_api_error_handling() {
        ...
        assert_eq!(
            result.expect_err("Expected error for no access token"),
            "No access token"
        );
    ...
    fn test_concurrent_state_access() {
        ...
        for handle in handles {
            handle
                .join()
                .expect("Thread panicked during concurrent access test");
        }
    }
