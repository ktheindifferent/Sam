...
pub fn cache_vwavs() {
    ...
    thread_manager::spawn_with_config(config, move |_shutdown_signal, _health_rx| {
        ...
        pg_query
            .queries
            .push(crate::memory::PGCol::String("HEARD".to_string()));
        pg_query
            .query_columns
            .push("observation_type =".to_string());
        pg_query
            .queries
            .push(crate::memory::PGCol::String("%PERSON%".to_string()));
        pg_query
            .query_columns
            .push(" AND observation_objects ilike".to_string());

        let observations =
            match crate::memory::Observation::select_lite(None, None, None, Some(pg_query)) {
                Ok(obs) => obs,
                Err(e) => {
                ...
                pool.execute(move || {
                    ...
                    if !Path::new(&cache_path).exists() {
                        ...
                        if Path::new(&xpath).exists() {
                            ...
                            crate::tools::safe_uinx_cmd("cp", &[&xpath, &tmp_file_path]);
                        } else {
                            let mut full_pg_query = crate::memory::PostgresQueries::default();
                            full_pg_query
                                .queries
                                .push(crate::memory::PGCol::String(th_obsv.oid.clone()));
                            full_pg_query.query_columns.push("oid =".to_string());

                            match crate::memory::Observation::select(
                                None,
                                None,
                                None,
                                Some(full_pg_query),
                        ...
                        if let Err(e) = crate::services::stt::patch_whisper_wts() {
...
pub fn observe(prediction: crate::services::stt::STTPrediction, file_path: &str) {
    let mut observation = crate::memory::Observation::new();
    observation.observation_type = crate::memory::ObservationType::HEARD;
    ...
    if !prediction.text.is_empty() {
        ...
        pg_query
            .queries
            .push(crate::memory::PGCol::String("Unknown Speaker".to_string()));
        pg_query.query_columns.push("oid ilike".to_string());
        let humans = match crate::memory::Human::select(None, None, None, Some(pg_query)) {
            Ok(h) => h,
            Err(e) => {
