...
lazy_static! {
    static ref CACHE_HIT_COUNTER: Counter =
        register_counter!("cache_hits_total", "Total number of cache hits").unwrap();
    static ref CACHE_MISS_COUNTER: Counter =
        register_counter!("cache_misses_total", "Total number of cache misses").unwrap();
    static ref CACHE_EVICTION_COUNTER: Counter =
        register_counter!("cache_evictions_total", "Total number of cache evictions").unwrap();
    ...
    .unwrap();
    static ref MEMORY_CACHE_SIZE: IntGauge =
        register_int_gauge!("memory_cache_size", "Current size of memory cache").unwrap();
...
impl HybridCache {
    pub async fn new(redis_pool: Pool, config: CacheConfig) -> Result<Self> {
        let memory_size =
            NonZeroUsize::new(config.memory_size).context("Invalid memory cache size")?;
    ...
    pub async fn get_or_load<T, F, E>(&self, key: &str, loader: F, ttl: Option<u64>) -> Result<T>
    where
        ...
        E: Into<anyhow::Error>,
    {
        ...
        debug!(
            "Cache miss and loaded for key: {} (took {:?})",
            key,
            start.elapsed()
        );
    ...
    where
        T: Serialize,
    {
        let serialized = serde_json::to_vec(item).context("Failed to serialize cached item")?;
    ...
    async fn release_lock(&self, lock_key: &str) -> Result<()> {
        ...
        let _: () = conn.del(lock_key).await.context("Failed to release lock")?;
    ...
    pub async fn invalidate(&self, pattern: &str) -> Result<()> {
        ...
        let _: () = conn
            .publish("cache_invalidation", pattern)
            .await
    ...
    pub async fn invalidate_key(&self, key: &str) -> Result<()> {
        ...
        // Publish invalidation event
        let _: () = conn
            .publish("cache_invalidation", key)
            .await
            .context("Failed to publish invalidation event")?;
    ...
    where
        ...
        F: Fn(String) -> std::pin::Pin<Box<dyn Future<Output = Result<T>> + Send>>
            + Send
            + Sync
            + 'static,
    {
        ...
        let tasks = keys.into_iter().map(|key| {
            ...
            tokio::spawn(async move {
                ...
                let result = cache
                    .get_or_load(&key, loader(key_clone), Some(cache.config.default_ttl))
                    .await;
    ...
    async fn setup_pubsub(&self) -> Result<PubSub> {
        ...
        Err(anyhow::anyhow!(
            "PubSub not yet implemented with deadpool_redis"
        ))
    }
