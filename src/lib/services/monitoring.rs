...
impl PerformanceMonitor {
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        self.metrics
            .increment_counter("operations_total", labels.clone())
            .await;
        ...
        match operation().await {
            Ok(result) => {
                ...
                self.metrics
                    .increment_counter("operations_success", labels.clone())
                    .await;

                // Record duration
                self.metrics
                    .record_histogram("operation_duration_ms", duration, labels)
                    .await;
            ...
            Err(e) => {
                ...
                self.metrics
                    .increment_counter("operations_failed", labels)
                    .await;
...
mod tests {
    ...
    async fn test_metrics_collector() {
        ...
        collector
            .increment_counter("requests", labels.clone())
            .await;
        collector
            .increment_counter("requests", labels.clone())
            .await;
        ...
        // Test gauge
        collector
            .set_gauge("memory_usage", 1024.0, HashMap::new())
            .await;
    ...
    async fn test_tracer() {
        ...
        tracer
            .add_tag(&span_id, "user_id".to_string(), "123".to_string())
            .await;
        tracer
            .add_log(
                &span_id,
