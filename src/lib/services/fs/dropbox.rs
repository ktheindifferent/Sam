...
pub fn get_db_obj() -> Result<crate::memory::config::Service, crate::services::Error> {
    let mut pg_query = crate::memory::PostgresQueries::default();
    pg_query
        .queries
        .push(crate::memory::PGCol::String("dropbox".to_string()));
...
pub fn update_key(key: String, refresh: Option<String>) -> Result<(), crate::services::Error> {
...
pub fn handle(
    ...
    if request.url() == "/api/services/dropbox/download" {
        let path_param = request.get_param("path").ok_or_else(|| {
            ...
            crate::http::Error::BadRequest("Missing path parameter".to_string())
        })?;
        let data = download_file(&path_param).map_err(|e| {
            log::error!("Failed to download file from dropbox: {}", e);
            crate::http::Error::InternalServerError(e.to_string())
        })?;
    ...
    if request.url() == "/api/services/dropbox/auth/2" {
        ...
        let new = auth.obtain_access_token(noc).map_err(|e| {
            ...
            crate::http::Error::InternalServerError("Failed to obtain access token".to_string())
        })?;
        let saved_key = auth.save().ok_or_else(|| {
            log::error!("Failed to save dropbox auth");
            crate::http::Error::InternalServerError("Failed to save auth".to_string())
        })?;
        update_key(saved_key, Some(new.refresh_token)).map_err(|e| {
            log::error!("Failed to update dropbox key: {}", e);
            crate::http::Error::InternalServerError("Failed to update key".to_string())
        })?;
...
pub fn create_sam_folder() -> Result<(), crate::services::Error> {
    create_folder("/Sam")
}

pub fn create_folder(path: &str) -> Result<(), crate::services::Error> {
    ...
    let auth = dropbox_sdk::oauth2::Authorization::load("ogyeqdms81svfke".to_string(), &obj.secret)
        .ok_or_else(|| {
            ...
            crate::services::Error::Other("Failed to load dropbox authorization".to_string())
        })?;
...
pub fn delete(path: &str) -> Result<(), crate::services::Error> {
    let obj = get_db_obj().map_err(|e| {
        log::error!("Failed to get dropbox database object: {}", e);
        crate::services::Error::Other(format!("Failed to get dropbox database object: {}", e))
    })?;
