...
use crate::memory::Thing;
use matc::{
    certmanager::{self, FileCertManager},
    clusters, controller, onboarding, tlv, transport,
};
...
impl MatterDeviceController {
    ...
    pub async fn set_device_on_off(thing: &Thing, on: bool, local_address: &str) -> Result<()> {
        let cert_path = "/opt/sam/keys/matter/";
        ...
        let res = con
            .invoke_request(endpoint, cluster, command_id, &[])
            .await?;
        res.tlv.dump(1);
        Ok(())
    }
    ...
    pub async fn set_device_level(thing: &Thing, level: u8, local_address: &str) -> Result<()> {
        let cert_path = "/opt/sam/keys/matter/";
        let controller_id = 100; // Default controller ID
        let device_id = thing
            .online_identifiers
...
pub fn handle(request: &rouille::Request) -> rouille::Response {
    ...
    match request.url().as_str() {
        "/api/things/matter/commissionable" if request.method() == "GET" => {
            ...
            match futures::executor::block_on(matc::discover::discover_commissionable(
                Duration::from_secs(timeout),
            )) {
                Ok(devices) => {
                    let result: Vec<CommissionableDevice> = devices
                        .into_iter()
                ...
                Err(e) => rouille::Response::text(format!("Discovery failed: {}", e))
        ...
        "/api/things/matter/pair" if request.method() == "POST" => {
            if let Ok(input) = json_input::<PairRequest>(request) {
                ...
                match futures::executor::block_on(fut) {
                    ...
                    Err(e) => rouille::Response::text(format!("Pairing failed: {}", e))
                        .with_status_code(500),
                }
            } else {
                ...
                match Thing::load_by_id(&input.thing_id) {
                    Ok(thing) => {
                        let local_address = input
                            .local_address
                            .unwrap_or_else(|| DEFAULT_LOCAL_ADDRESS.to_string());
                        let fut = MatterDeviceController::set_device_on_off(
                            &thing,
                            input.on,
                            &local_address,
                        );
                        match futures::executor::block_on(fut) {
                            ...
                            Err(e) => rouille::Response::text(format!("On/Off failed: {}", e))
                                .with_status_code(500),
                        }
                    }
                    Err(e) => rouille::Response::text(format!("Thing not found: {}", e))
                ...
                match Thing::load_by_id(&input.thing_id) {
                    Ok(thing) => {
                        let local_address = input
                            .local_address
                            .unwrap_or_else(|| DEFAULT_LOCAL_ADDRESS.to_string());
                        let fut = MatterDeviceController::set_device_level(
                            &thing,
                            input.level,
                            &local_address,
                        );
                        match futures::executor::block_on(fut) {
                            Ok(_) => rouille::Response::text("Success"),
                            Err(e) => rouille::Response::text(format!("Set level failed: {}", e))
                                .with_status_code(500),
                        }
                    }
                    Err(e) => rouille::Response::text(format!("Thing not found: {}", e))
