...
pub fn init_cache() {
    let cache_thread = thread::Builder::new()
        ...
        .spawn(move || {
            loop {
                let _ = crate::memory::storage::File::cache_all();
...
pub fn handle(
    ...
    if request.url() == "/api/services/storage/locations" {
        ...
        if request.method() == "POST" {
            ...
            let mut location = crate::memory::config::FileStorageLocation::new();
    ...
    if request.url() == "/api/services/storage/files" {
        // Handle file storage
        if request.method() == "GET" {
            let files = crate::memory::storage::File::select_lite(None, None, None, None)?;
        ...
        if request.method() == "POST" {
            ...
            file.storage_location_oid = input
                .storage_location_oid
    ...
    if request.url().contains("/api/services/storage/file/") {
        ...
        if let Some(oid) = request.url().split('/').nth(5) {
