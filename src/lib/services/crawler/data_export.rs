...
impl DataExporter {
    ...
    pub async fn export_to_file(path: &Path, options: ExportOptions) -> Result<ExportInfo> {
        info!(
            "Starting data export to {:?} with format {:?}",
            path, options.format
        );
        ...
        info!(
            "Export completed: {} records written to {:?}",
            data.pages.len(),
            path
        );
    ...
    pub async fn export_to_bytes(options: ExportOptions) -> Result<Vec<u8>> {
        let data = Self::fetch_data(&options).await?;
...
"#,
        ...
        html.push_str("<table>\n");
        html.push_str(
            "<tr><th>URL</th><th>Title</th><th>Status</th><th>Type</th><th>Size</th></tr>\n",
        );
