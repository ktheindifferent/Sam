...
impl Default for JsRendererConfig {
    fn default() -> Self {
        Self {
            ...
            blocked_resources: vec![ResourceType::Image, ResourceType::Font, ResourceType::Media],
            custom_scripts: vec![],
            enable_cache: false,
            enable_cookies: true,
            proxy: None,
...
impl JsRenderer {
    ...
    pub async fn initialize(&self) -> Result<(), JsRenderError> {
        info!(
            "Initializing JavaScript renderer with {} browser instances",
    ...
    async fn get_browser_instance(&self) -> Result<Arc<BrowserInstance>, JsRenderError> {
        ...
        Err(JsRenderError::BrowserPoolExhausted(
            "All browsers in use".to_string(),
        ))
    }
    ...
    async fn create_browser_instance(&self, id: usize) -> Result<BrowserInstance, JsRenderError> {
        info!(
            "Creating browser instance {} with engine {:?}",
            id, self.config.engine
        );
    ...
    async fn render_with_chrome(
        ...
        url: &str,
    ) -> Result<RenderResult, JsRenderError> {
    ...
    async fn render_with_firefox(
        ...
        Err(JsRenderError::NotImplemented(
            "Firefox rendering not yet implemented".to_string(),
        ))
    }

    /// Render with Safari
    async fn render_with_safari(
        &self,
        browser: &BrowserInstance,
        url: &str,
    ) -> Result<RenderResult, JsRenderError> {
        ...
        Err(JsRenderError::NotImplemented(
            "Safari rendering not yet implemented".to_string(),
        ))
    }
...
static GLOBAL_JS_RENDERER: Lazy<Arc<RwLock<Option<JsRenderer>>>> =
...
pub async fn render_with_javascript(url: &str) -> Result<RenderResult, JsRenderError> {
    ...
    match global.as_ref() {
        ...
        None => Err(JsRenderError::NotImplemented(
