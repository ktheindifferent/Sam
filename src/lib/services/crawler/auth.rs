...
impl AuthManager {
    ...
    pub async fn apply_auth(
        ...
        match auth {
            ...
            AuthMethod::Basic { username, password } => {
                ...
                headers.insert(
                    AUTHORIZATION,
                    HeaderValue::from_str(&auth_value).context("Invalid Basic auth header")?,
                );
            ...
            AuthMethod::Bearer { token } => {
                ...
                headers.insert(
                    AUTHORIZATION,
                    HeaderValue::from_str(&auth_value).context("Invalid Bearer token header")?,
                );
            ...
            AuthMethod::Cookie { cookies } => {
                ...
                headers.insert(
                    COOKIE,
                    HeaderValue::from_str(&cookie_string).context("Invalid cookie header")?,
                );
            ...
            AuthMethod::OAuth {
                ...
                let token = self
                    .get_or_refresh_oauth_token(domain, client_id, client_secret, token_url, scopes)
                    .await?;

                let auth_value = format!("{} {}", token.token_type, token.access_token);
                headers.insert(
                    AUTHORIZATION,
                    HeaderValue::from_str(&auth_value).context("Invalid OAuth header")?,
                );
    ...
    async fn get_or_refresh_oauth_token(
        ...
        let token = self
            .request_oauth_token(client_id, client_secret, token_url, scopes)
            .await?;
...
static GLOBAL_AUTH_MANAGER: Lazy<AuthManager> = Lazy::new(|| AuthManager::new());
