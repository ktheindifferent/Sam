...
impl OptimizedUrlTracker {
    ...
    pub fn new(expected_urls: usize, cache_size: usize) -> Self {
        ...
        let cache_size =
            NonZeroUsize::new(cache_size.max(1000)).expect("Cache size must be non-zero");
    ...
    pub async fn push(&self, url: String, depth: usize) -> Result<()> {
        ...
        if queue.len() >= self.max_memory_items {
            if let Some(pool) = &self.redis_pool {
                ...
                if queue.len() == self.max_memory_items {
                    warn!(
                        "URL queue at maximum capacity ({}), new URLs may be dropped",
...
mod tests {
    ...
    async fn test_bounded_queue() {
        ...
        // Test push and pop
        queue
            .push("https://example.com".to_string(), 0)
            .await
            .unwrap();
