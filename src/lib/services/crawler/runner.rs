...
use crate::services::crawler::job::CrawlJob;
...
fn load_dns_cache(should_use_redis: bool) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    Box::pin(async move {
        if crate::services::redis::is_running().await && should_use_redis {
...
async fn crawl_url_inner(
    ...
    let parsed_url = match crate::security::validate_url(&url) {
    ...
    // pg_query.queries.push(crate::memory::PGCol::String(format!("{}",url.clone())));
    ...
    //     return Err(crate::memory::Error::from_kind(crate::memory::ErrorKind::Msg(
    ...
    let resp: Result<reqwest::Response, crate::memory::Error> = match resp {
        Some(r) => Ok(r),
        None => Err(crate::memory::Error::Other(format!(
...
pub async fn run_crawler_service() -> crate::memory::Result<()> {
    ...
    loop {
        ...
        if let Some(mut job) = jobs.into_iter().next() {
                                    ...
                                    if should_add {
                                            ...
                                            || !mime_type_from_url(link).contains("text/")
                                        {
                                            ...
                                            static JOB_BATCH: once_cell::sync::Lazy<
                                                tokio::sync::Mutex<
                                                    Vec<crate::memory::cache::WebCrawl>,
                                                >,
                                            ...
                                            });
                                            {
                                                ...
                                                if batch.len() >= 1000 {
                                                    ...
                                                    if let Err(e) =
                                                        crate::memory::cache::WebCrawl::save_batch_async(jobs_to_save)
                                                            .await
                                                    {
                                                        log::warn!(
            ...
            crate::services::crawler::job::CrawlJob::destroy_async(job.oid.clone())
                .await
                .unwrap_or_else(|_| {
...
fn validate_url(url: &str) -> crate::memory::Result<()> {
    if !is_valid_url(url) {
        Err(crate::memory::Error::Other("Invalid URL".to_string()).into())
    } else {
        Ok(())
    }
}
...
) -> crate::memory::Result<(Option<&'static str>, Vec<String>)> {
