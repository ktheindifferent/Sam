...
impl CrawledContent {
    ...
    pub fn detect_language(text: &str) -> Option<String> {
        ...
        let mut scores = [
            ...
            (
                "fr",
                french_words.iter().filter(|&&w| words.contains(&w)).count(),
            ),
            (
                "de",
                german_words.iter().filter(|&&w| words.contains(&w)).count(),
            ),
        ];
    ...
    pub async fn save_batch(contents: Vec<Self>) -> Result<(usize, usize)> {
        ...
        info!(
            "Saved {} new pages, deduplicated {} pages",
            saved, deduplicated
        );
    ...
    pub async fn get_dedup_stats() -> Result<DeduplicationStats> {
        ...
        let total_urls: i64 = client
            .query_one("SELECT COUNT(DISTINCT url) FROM crawled_content", &[])
            .await?
            .get(0);

        let unique_content: i64 = client
            .query_one(
...
mod tests {
    ...
    fn test_compression() {
        ...
        let content =
            CrawledContent::new("https://example.com".to_string(), "Test", Some(&html), 200);
    ...
    fn test_title_extraction() {
        let html =
            r#"<html><head><title>Test Page Title</title></head><body>Content</body></html>"#;
    ...
    fn test_language_detection() {
        ...
        assert_eq!(
            CrawledContent::detect_language(english),
            Some("en".to_string())
        );

        let spanish = "El rápido zorro marrón salta sobre el perro perezoso";
        assert_eq!(
            CrawledContent::detect_language(spanish),
            Some("es".to_string())
        );
    }
    ...
    fn test_clean_content_with_unicode() {
        ...
        let mixed = format!(
            "data:image/png;base64,{} normal_text \u{200d}\u{200b}",
            "A".repeat(3000)
        );
