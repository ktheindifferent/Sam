    ...
    fn update_stats(&mut self, response_time_ms: u64) {
        ...
        if self.avg_response_time_ms == 0.0 {
            ...
            self.avg_response_time_ms =
                alpha * response_time_ms as f64 + (1.0 - alpha) * self.avg_response_time_ms;
        }
    }
    ...
    fn adapt_delay(&mut self, response_time_ms: u64, config: &RateLimitConfig) {
        ...
        if response_time_ms > config.slow_threshold_ms {
            ...
            self.current_delay_ms =
                (self.current_delay_ms as f64 * config.slow_response_factor) as u64;
            debug!(
                "Slow response ({}ms), increasing delay to {}ms",
            ...
            self.current_delay_ms =
                (self.current_delay_ms as f64 * config.fast_response_factor) as u64;
            debug!(
                "Fast response ({}ms), decreasing delay to {}ms",
                response_time_ms, self.current_delay_ms
            );
        }
    ...
    pub async fn wait_for_slot(&self, url: &str, robots_delay_seconds: Option<f64>) -> Result<()> {
        ...
        if let Some(retry_after) = state.retry_after {
            if Instant::now() < retry_after {
                ...
                info!(
                    "Respecting Retry-After header for {}: waiting {:?}",
                    domain, wait_time
                );
                sleep(wait_time).await;
        ...
        if elapsed < required_delay {
            ...
            debug!(
                "Rate limiting {}: waiting {:?} (delay: {}ms)",
            ...
            match status_code {
                Some(429) | Some(503) => {
                    ...
                    state.current_delay_ms =
                        (state.current_delay_ms * 2).min(self.config.max_delay_ms);
                    ...
                    if let Some(retry_seconds) = retry_after_header {
                        ...
                        info!(
                            "Set retry-after for {} to {} seconds",
                            domain, retry_seconds
                        );
                    }
                }
...
static RATE_LIMITER: once_cell::sync::OnceCell<AdaptiveRateLimiter> =
...
pub fn get_rate_limiter() -> &'static AdaptiveRateLimiter {
    RATE_LIMITER.get_or_init(|| AdaptiveRateLimiter::new(RateLimitConfig::default(), Some(100)))
}
...
mod tests {
    ...
    async fn test_adaptive_delay() {
        ...
        limiter.wait_for_slot(url, None).await.unwrap();
        limiter
            .record_request_complete(url, Duration::from_millis(6000), Some(200), None)
            .await
            .unwrap();
