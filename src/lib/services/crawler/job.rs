...
impl CrawlJob {
    ...
    pub fn select(
        ...
        for j in jsons {
            let object: Self = serde_json::from_str(&j).map_err(|e| {
                crate::memory::Error::Other(format!("Failed to deserialize CrawlJob: {e}"))
            })?;
    ...
    pub async fn select_async(
        ...
        let result = tokio::task::spawn_blocking(move || Self::select(limit, offset, order, query))
            .await
            .map_err(|e| {
                crate::memory::Error::Other(format!("JoinError in select_async: {e}"))
            })??;
        Ok(result)
    }
    /// Save to DB (insert or update by oid).
    pub fn save(&self) -> crate::memory::Result<Self> {
    ...
    pub async fn save_batch_async(jobs: &[Self]) -> crate::memory::Result<()> {
        if jobs.is_empty() {
            return Ok(());
        }
        let config = crate::memory::Config::new();
    ...
    pub async fn destroy_async(oid: String) -> crate::memory::Result<bool> {
