...
pub async fn install() {
    ...
    if !is_installed() {
        ...
        if let Err(e) = install_docker() {
            report_service_error("docker", &format!("Failed to install Docker: {e}"), None);
            return;
        }
    } else {
...
pub async fn start() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        match output {
            ...
            _ => {
                let err =
                    anyhow::anyhow!("Failed to start Docker Desktop. Please start it manually.");
                error!("{}", err);
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match output {
            ...
            _ => {
                let err =
                    anyhow::anyhow!("Failed to start Docker daemon. Please start it manually.");
                error!("{}", err);
                report_service_error("docker", &err, None);
            }
        }
...
fn install_docker() -> Result<()> {
    let status = Command::new("brew")
        ...
        .status()
        .context("Failed to launch Homebrew Docker installer")?;

    if status.success() {
        Ok(())
    } else {
        Err(anyhow::anyhow!(
            "Homebrew Docker installer exited with status {status}"
        ))
    }
}
...
fn install_docker() -> Result<()> {
    ...
    if status.success() {
        ...
        Err(anyhow::anyhow!(
            "Docker install script exited with status {status}"
        ))
    }
}
...
fn install_docker() -> Result<()> {
    let status = Command::new("powershell")
        ...
        .status()
        .context("Failed to launch winget Docker installer")?;

    if status.success() {
        Ok(())
    } else {
        Err(anyhow::anyhow!(
            "winget Docker installer exited with status {status}"
        ))
    }
}
