...
impl BackupService {
    ...
    pub async fn perform_full_backup(&self) -> Result<BackupMetadata, BackupError> {
        ...
        let final_path = if self.config.compression.enabled {
            ...
            let compressed_size = self
                .create_compressed_archive(&backup_path, &archive_path)
                .await?;
        ...
        metadata.duration_seconds = Some((Utc::now() - start_time).num_seconds() as u64);
    ...
    pub async fn perform_incremental_backup(
        ...
        info!(
            "Starting incremental backup: {} (parent: {})",
            backup_name, parent_backup_id
        );
    ...
    pub async fn restore_backup(
        ...
        let extracted_path = if metadata.compressed_size_bytes.is_some() {
            ...
            self.extract_compressed_archive(&backup_path, &temp_path)
                .await?;
            temp_path
        } else {
            backup_path
        ...
        for target_name in &metadata.targets {
            let (files, bytes) = self
                .restore_target(target_name, &extracted_path, &restore_path)
                .await?;
    ...
    async fn backup_target(
        ...
        match &target.target_type {
            BackupTargetType::Database => self.backup_database(&target_path).await,
            BackupTargetType::FileSystem => self.backup_filesystem(target, &target_path).await,
            BackupTargetType::Configuration => self.backup_filesystem(target, &target_path).await,
            BackupTargetType::Redis => self.backup_redis(&target_path).await,
            BackupTargetType::Custom(cmd) => self.backup_custom(cmd, &target_path).await,
        }
    }
    ...
    async fn backup_database(&self, target_path: &Path) -> Result<(u64, u64), BackupError> {
        ...
        let dump_file = dump_file.to_str().ok_or_else(|| {
            BackupError::CommandFailed("Database dump path is not valid UTF-8".to_string())
        })?;
    ...
    async fn backup_filesystem(
        ...
        for include_path in &target.include_paths {
            if include_path.exists() {
                let (size, count) = self
                    .copy_directory(include_path, target_path, &target.exclude_patterns)
                    .await?;
    ...
    async fn backup_redis(&self, target_path: &Path) -> Result<(u64, u64), BackupError> {
        ...
        Command::new("redis-cli").args(["BGSAVE"]).output()?;
    ...
    fn list_directory_recursive(
        ...
        Box<dyn std::future::Future<Output = Result<Vec<String>, BackupError>> + Send + '_>,
    > {
        Box::pin(Self::list_directory_recursive_impl(path.to_path_buf()))
    }

    /// Implementation for recursive directory listing  
    fn list_directory_recursive_impl(
        path: PathBuf,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<String>, BackupError>> + Send>>
    {
        Box::pin(async move {
    ...
    fn get_backup_path(&self, metadata: &BackupMetadata) -> PathBuf {
        if metadata.compressed_size_bytes.is_some() {
            self.config
                .base_path
                .join(format!("{}.tar.gz", metadata.name))
        } else {
