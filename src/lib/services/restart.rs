    ...
    async fn notify(&self, event: RestartEvent) -> Result<()> {
        match event {
            RestartEvent::RestartInitiated {
                ...
                info!(
                    "Initiating restart for {:?} (attempt {}): {}",
            ...
            RestartEvent::RestartFailed {
                ...
                error!(
                    ...
                    service, attempt, error
                );
            }
            RestartEvent::CircuitBreakerTripped {
                service,
                failure_count,
            } => {
                error!(
                    "Circuit breaker tripped for {:?} after {} failures",
            ...
            RestartEvent::DependencyCheckFailed {
                service,
                missing_dependencies,
            } => {
                warn!(
                    "Dependency check failed for {:?}, missing: {:?}",
                    service, missing_dependencies
                );
            }
...
impl RestartManager {
        ...
        match config.strategy {
            ...
            RestartStrategy::ExponentialBackoff {
                ...
                let delay_ms = (base_delay.as_millis() as f64
                    * (multiplier as f64).powi(attempt as i32))
                    as u64;
    ...
    pub fn update_circuit_breaker(&self, service: &ServiceName, success: bool) {
        ...
        let mut metrics = match self.metrics.write() {
            Ok(guard) => guard,
            Err(e) => {
                error!("Failed to acquire restart metrics lock: {}", e);
                return;
            }
        };
        let configs = match self.configs.read() {
            Ok(guard) => guard,
            Err(e) => {
                error!("Failed to acquire restart configs lock: {}", e);
                return;
            }
        };
        let config = configs.get(service).cloned().unwrap_or_default();
    ...
    pub fn update_metrics(&self, service: &ServiceName, success: bool, duration: Duration) {
        ...
        let m = metrics
            .entry(service.clone())
            .or_insert_with(RestartMetrics::default);
        ...
        if success {
            ...
            let total_time =
                m.average_restart_time.as_millis() as u64 * (m.successful_restarts - 1);
...
mod tests {
    ...
    fn test_exponential_backoff_calculation() {
        ...
        manager
            .register_config(ServiceName::Redis, config)
            .expect("Failed to register config");

        // Test backoff progression
        assert_eq!(
            manager.calculate_delay(&ServiceName::Redis, 0),
            Duration::from_secs(1)
        );
        assert_eq!(
            manager.calculate_delay(&ServiceName::Redis, 1),
            Duration::from_secs(2)
        );
        assert_eq!(
            manager.calculate_delay(&ServiceName::Redis, 2),
            Duration::from_secs(4)
        );
        assert_eq!(
            manager.calculate_delay(&ServiceName::Redis, 3),
            Duration::from_secs(8)
        );
        assert_eq!(
            manager.calculate_delay(&ServiceName::Redis, 4),
            Duration::from_secs(16)
        );
        assert_eq!(
            manager.calculate_delay(&ServiceName::Redis, 5),
    ...
    fn test_circuit_breaker_state_transitions() {
        ...
        manager
            .register_config(ServiceName::PostgreSQL, config)
            .expect("Failed to register config");
    ...
    fn test_metrics_tracking() {
        let manager = RestartManager::new();
        manager
            .register_config(ServiceName::Docker, RestartConfig::default())
            .expect("Failed to register config");
        ...
        let metrics = manager
            .get_metrics(&ServiceName::Docker)
        ...
        let metrics = manager
            .get_metrics(&ServiceName::Docker)
            .expect("Failed to get metrics after failure");
