...
impl ApiServer {
    pub fn new(config: Config) -> Result<Self, LifxError> {
        config.validate()
            .map_err(|e| LifxError::ConfigError(e))?;

        let source = 0x72757374; // "rust" in hex
        let discovery = Arc::new(Mutex::new(
            DiscoveryService::new(source)
                .map_err(|e| LifxError::DiscoveryFailed(e.to_string()))?
        ));

        info!("LIFX discovery service initialized with source: 0x{:08x}", source);
            ...
            while !stop_flag_bg.load(Ordering::SeqCst) && !shutdown_signal.load(Ordering::Relaxed) {
                match discovery_clone.lock() {
                    ...
                    Err(e) => {
                        ...
                        warn!("LIFX refresh failed ({} / {}): {}", consecutive_failures, MAX_FAILURES, e);
