...
pub mod safe_ops {
    ...
    pub fn compile_regex(pattern: &str) -> Result<Regex, WebSocketError> {
        Regex::new(pattern).map_err(|e| {
            WebSocketError::RegexCompilation(format!(
    ...
    pub fn compile_regex_or_default(pattern: &str, default_pattern: &str) -> Regex {
        match Regex::new(pattern) {
            ...
            Err(e) => {
                log::error!(
                    "Failed to compile regex '{}': {}, using default pattern",
                    pattern,
                    e
                );
                Regex::new(default_pattern).expect("Default regex pattern should be valid")
            }
        }
    }
