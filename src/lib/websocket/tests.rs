...
mod error_handling_tests {
    use super::super::error::{safe_ops, WebSocketError, WsSecurityError};
    use super::super::security::{MessageValidator, SessionManager, WebSocketSecurityConfig};
    ...
    async fn test_session_manager_error_handling() {
        ...
        let result = session_manager
            .reauthenticate("client1", invalid_token)
            .await;
        assert!(result.is_err());

        // Test session validation for non-existent client
        let missing_session = session_manager
            .validate_session("non_existent_client")
            .await;
        assert!(result.is_err());
    }
...
mod integration_tests {
    use super::super::security::{
        ConnectionTracker, MessagePriority, MessageQueue, WebSocketLimits, WebSocketSecurityConfig,
        WsRateLimiter,
    };
    use std::net::IpAddr;
    ...
    async fn test_connection_limits_with_safe_ip_parsing() {
        ...
        assert_eq!(fallback_ip.to_string(), "127.0.0.1");
        assert!(tracker
            .add_connection(fallback_ip, "fallback_client".to_string())
            .await
            .is_ok());
    }
    ...
    async fn test_message_queue_with_priority() {
        ...
        assert!(queue
            .enqueue("client1", "low".to_string(), MessagePriority::Low)
            .await
            .is_ok());
        assert!(queue
            .enqueue("client1", "critical".to_string(), MessagePriority::Critical)
            .await
            .is_ok());
        assert!(queue
            .enqueue("client1", "normal".to_string(), MessagePriority::Normal)
            .await
            .is_ok());
        assert!(queue
            .enqueue("client1", "high".to_string(), MessagePriority::High)
            .await
            .is_ok());
