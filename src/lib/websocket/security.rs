...
impl WsRateLimiter {
    ...
    pub async fn check_rate_limit(&self, client_id: &str) -> Result<(), WsSecurityError> {
        ...
        if bucket.count > self.config.max_messages_per_minute {
            ...
            warn!(
                "Rate limit exceeded for client {}: {} violations",
                client_id, bucket.violations
            );
...
impl ConnectionTracker {
    ...
    pub async fn add_connection(
        ...
        ip_connections.retain(|conn| {
            now.duration_since(conn.connected_at)
        ...
        info!(
            "New connection from IP {}: total connections {}",
            ip,
            ip_connections.len()
        );
        Ok(())
...
impl MessageValidator {
    ...
    pub fn validate_message(&self, msg: &str) -> Result<(), WsSecurityError> {
        ...
        if let Err(e) = serde_json::from_str::<serde_json::Value>(msg) {
            return Err(WsSecurityError::MessageValidationFailed(format!(
                "Invalid JSON: {}",
                e
            )));
        }
    ...
    pub fn validate_command(
        ...
        if restricted_commands.contains(&command)
            && !user_permissions.contains(&format!("command:{}", command))
        {
            return Err(WsSecurityError::UnauthorizedAction(format!(
...
impl SessionManager {
    ...
    pub async fn reauthenticate(
        ...
        if claims.client_id != client_id {
            error!(
                "Client ID mismatch: token has {} but request has {}",
    ...
    pub async fn authenticate_with_token(
        ...
        info!(
            "Authenticated new session with token for client {}",
            claims.client_id
        );
        ...
        match decode::<JwtClaims>(token, &key, &validation) {
            Ok(token_data) => {
                ...
                if let Some(nbf) = token_data.claims.nbf {
                    if nbf > now {
                        error!(
                            "Token not yet valid for client {}",
                ...
                info!(
                    "Token validated successfully for client {}",
                    token_data.claims.client_id
                );
            ...
            Err(err) => match err.kind() {
                ...
                _ => {
                    ...
                    Err(WsSecurityError::InvalidToken(format!(
                        "Validation failed: {}",
                        err
                    )))
                }
    ...
    pub fn generate_token(
        ...
        match encode(&header, &claims, &key) {
            Ok(token) => {
                info!(
                    "Generated token for client {} (user: {})",
                    client_id, user_id
                );
                Ok(token)
            }
...
impl MessageQueue {
    ...
    pub async fn enqueue(
        ...
        if queue.len() >= self.max_size {
            // Apply backpressure
            warn!(
                "Message queue full for client {}, applying backpressure",
                client_id
            );
...
impl WebSocketLimits {
    ...
    pub async fn validate_connection(
        ...
        self.connection_tracker
            .add_connection(ip, client_id.clone())
            .await?;
        ...
        let session = if let Some(token) = token {
            ...
            warn!(
                "No token provided for client {}, creating unauthenticated session",
                client_id
            );
...
mod tests {
    ...
    async fn test_connection_tracker() {
        ...
        assert!(tracker
            .add_connection(ip, "client1".to_string())
            .await
            .is_ok());
        assert!(tracker
            .add_connection(ip, "client2".to_string())
            .await
            .is_ok());

        // 3rd connection should fail
        assert!(tracker
            .add_connection(ip, "client3".to_string())
            .await
            .is_err());
    ...
    async fn test_message_queue() {
        ...
        // Enqueue messages
        assert!(queue
            .enqueue("client1", "msg1".to_string(), MessagePriority::Normal)
            .await
            .is_ok());
        assert!(queue
            .enqueue("client1", "msg2".to_string(), MessagePriority::High)
            .await
            .is_ok());
        assert!(queue
            .enqueue("client1", "msg3".to_string(), MessagePriority::Low)
            .await
            .is_ok());

        // Queue should be full
        assert!(queue
            .enqueue("client1", "msg4".to_string(), MessagePriority::Normal)
            .await
            .is_err());
