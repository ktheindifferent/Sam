...
pub fn validate_command(command: &str) -> bool {
    ...
    let allowed_prefixes = vec![
        "ls", "pwd", "cd", "cat", "less", "head", "tail", "grep", "find", "mkdir", "touch", "cp",
        "mv", "rm", "chmod", "echo", "wc", "sort", "redis", "spotify", "lifx", "docker", "crawler",
        "pg", "postgres", "tts", "llama", "status", "help", "clear", "df", "du", "ps", "top",
...
mod tests {
    ...
    fn test_parse_natural_language() {
        assert_eq!(
            parse_natural_language("clear my downloads folder"),
            Some("rm -rf ~/Downloads/*".to_string())
        );
        assert_eq!(
            parse_natural_language("show me the current directory"),
            Some("pwd".to_string())
        );
        assert_eq!(
            parse_natural_language("start redis service"),
            Some("redis start".to_string())
        );
    }
