...
pub async fn execute_action_with_context(command: &str, context: ExecutionContext) -> ActionResult {
    ...
    ActionResult {
        success: !has_error,
        output: output_text,
        error: if has_error { Some("Command execution had errors".to_string()) } else { None },
    }
}
...
pub async fn execute_natural_language_action(natural_text: &str) -> ActionResult {
    match command_parser::parse_natural_language(natural_text) {
        Some(command) => {
            log::info!("Parsed natural language '{}' to command '{}'", natural_text, command);
            execute_action(&command).await
        }
        None => ActionResult {
            success: false,
            output: String::new(),
            error: Some(format!("Could not parse natural language request: {}", natural_text)),
        }
    }
}
...
pub fn is_safe_for_auto_execution(command: &str) -> bool {
    ...
    let safe_commands = vec![
        "ls", "pwd", "cat", "less", "head", "tail", "grep", "find",
        "echo", "wc", "sort", "date", "whoami", "uname", "df", "du", "ps", "top",
        "status", "help", "clear"
    ];
    
    let safe_service_commands = vec![
        "redis status", "spotify status", "lifx status", "docker status", 
        "crawler status", "pg status", "postgres status"
    ];
    ...
    if safe_service_commands.iter().any(|safe_cmd| command.starts_with(safe_cmd)) {
        return true;
    }
...
pub fn get_execution_confirmation(command: &str) -> String {
    if is_safe_for_auto_execution(command) {
        ...
        format!("⚠️  About to execute: {} - This command will make changes to your system.", command)
    }
}
