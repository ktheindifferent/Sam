...
use crate::resource_management::{TempFile, CleanupGuard, ResourceCleanup};
...
fn handle_list_observations(request: &Request) -> Result<Response, crate::http::Error> {
...
fn handle_get_observation_file(request: &Request) -> Result<Response, crate::http::Error> {
    ...
    let wav_data = observation.observation_file
        .ok_or_else(|| crate::http::Error::from(
...
fn handle_visual_wav(request: &Request) -> Result<Response, crate::http::Error> {
    let oid = extract_oid_from_url(request.url(), 4)?;
    ...
    if cache_path.exists() {
        ...
        let data = std::fs::read(&cache_path)
            .map_err(|e| crate::http::Error::from(e))?;
...
async fn process_visual_wav_async(oid: String, wav_data: Vec<u8>) -> Result<Vec<u8>> {
    ...
    crate::services::stt::patch_whisper_wts(wts_path.clone())
...
fn extract_oid_from_url(url: &str, index: usize) -> Result<String, crate::http::Error> {
...
fn get_observation_by_oid(oid: &str) -> Result<crate::memory::Observation, crate::http::Error> {
    let mut pg_query = crate::memory::PostgresQueries::default();
    pg_query.queries.push(crate::memory::PGCol::String(oid.to_string()));
