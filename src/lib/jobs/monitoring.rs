...
impl JobMonitor {
    ...
    pub async fn record_success(&self, job_id: &str, processing_time: Duration) {
        ...
        metrics
            .processing_times
            .push(processing_time.as_millis() as f64);
    ...
    async fn persist_metrics(redis_pool: &Pool, metrics: &Arc<RwLock<Metrics>>) -> Result<()> {
        ...
        let snapshot_json =
            serde_json::to_string(&snapshot).context("Failed to serialize metrics snapshot")?;
    ...
    async fn check_alerts(redis_pool: &Pool, metrics: &Arc<RwLock<Metrics>>) -> Result<()> {
        ...
        if metrics.total_timeouts > 10 {
            info!(
                "ALERT: High number of job timeouts: {}",
    ...
    pub async fn record_event(
        ...
        let event_json = serde_json::to_string(&event).context("Failed to serialize job event")?;
