...
use super::types::{JobError, JobResult, Priority};
                    ...
                    Ok(Some(job)) => {
                        ...
                        match handler {
                            Some(h) => {
                                ...
                                let result =
                                    timeout(timeout_duration, handler.handle(job.payload.clone()))
                                        .await;
                                ...
                                match result {
                                    Ok(Ok(JobResult::Success(value))) => {
                                        // Job succeeded
                                        if let Err(e) =
                                            queue.complete_job(job.clone(), value.clone()).await
                                        {
                                            error!(
                                        ...
                                        if let Err(e) = handler
                                            .on_success(&job.payload, &JobResult::Success(value))
                                            .await
                                        {
                                    ...
                                    Ok(Ok(JobResult::Failure(error))) => {
                                        ...
                                        if let Err(e) =
                                            queue.fail_job(job.clone(), error.clone()).await
                                        {
                                            error!(
                                        ...
                                        if let Err(e) = handler
                                            .on_failure(
                                                &job.payload,
                                                &JobError::ExecutionFailed(error),
                                            )
                                            .await
                                        {
                                    ...
                                    Ok(Ok(JobResult::Retry(error))) => {
                                        ...
                                        if job.should_retry() {
                                            if let Err(e) = queue
                                                .retry_job(job.clone(), error_msg.clone())
                                                .await
                                            {
                                            ...
                                            if let Err(e) = handler
                                                .on_retry(
                                                    &job.payload,
                                                    job.retry_count,
                                                    &JobError::ExecutionFailed(error_msg),
                                                )
                                                .await
                                            {
                                                error!(
                                                    "Job {} on_retry hook failed: {}",
                                                    job.id, e
                                                );
                                            }
                                            ...
                                            if let Err(e) =
                                                queue.fail_job(job.clone(), error_msg).await
                                            {
                                                error!(
                                                    "Failed to mark job {} as failed: {}",
                                                    job.id, e
...
impl WorkerPool {
    ...
    pub async fn resize(&self, new_size: usize) -> Result<()> {
        ...
        if new_size > current_size {
            ...
            info!(
                "Worker pool expanded from {} to {} workers",
            ...
            info!(
                "Worker pool reduced from {} to {} workers",
                current_size, new_size
            );
        }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WorkerPool")
            .field(
                ...
                &format!(
                    "<{} workers>",
                    self.num_workers.load(std::sync::atomic::Ordering::Relaxed)
                ),
            )
            .field("queue", &"<JobQueue>")
            .field(
