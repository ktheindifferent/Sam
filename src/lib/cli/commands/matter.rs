use crate::cli::spinner::run_with_spinner;
use crate::memory::Thing;
use crate::services::matter::MatterDeviceController;
use std::sync::Arc;
        ...
        Some("pair") => {
            ...
            if let (Some(device_address), Some(pin)) = (device_address, pin) {
                ...
                run_with_spinner(
                    ...
                    move || async move {
                        match MatterDeviceController::pair_device(
                            &device_address,
                            &pin,
                            &local_address,
                        )
                        ...
                        {
                            Ok(thing) => {
                                format!("Paired Matter device: {} (oid: {})", thing.name, thing.oid)
                            }
                ...
                if let (Some(oid), Some(state)) = (oid, state) {
                    let on = match state.to_lowercase().as_str() {
                        ...
                        _ => {
                            let mut out = output_lines.lock().await;
                            out.push(
                                "Usage: matter set power <thing_oid> <on|off> [local_address]"
                                    .to_string(),
                            );
                            return;
                        }
                    ...
                    run_with_spinner(
                        &output_lines,
                        &format!(
                            "Setting device {oid} power {}...",
                            if on { "ON" } else { "OFF" }
                        ),
                        |lines, msg| lines.push(msg.to_string()),
                        move || async move {
                            let thing = Thing::select_async(None, None, None, None)
                                .await
                                .unwrap_or_default()
                                .into_iter()
                                .find(|t| t.oid == oid);
                            match thing {
                                Some(t) => {
                                    match MatterDeviceController::set_device_on_off(
                                        &t,
                                        on,
                                        &local_address,
                                    )
                        ...
                        move || async move {
                            match level_str.parse::<u8>() {
                                Ok(level) => {
                                    ...
                                    match thing {
                                        Some(t) => {
                                            match MatterDeviceController::set_device_level(
                                                &t,
                                                level,
                                                &local_address,
                                            )
                    ...
                    match discover::discover_commissionable(Duration::from_secs(timeout)).await {
                        Ok(infos) => {
                            if infos.is_empty() {
                                ...
                                let mut msg =
                                    format!("Discovered {} Matter device(s):\n", infos.len());
                                for info in infos {
                                    msg.push_str(&format!("{:#?}\n", info));
                                }
                                msg
