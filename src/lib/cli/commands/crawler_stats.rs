...
pub async fn handle_crawler_stats(cmd: &str, output_lines: &Arc<Mutex<Vec<String>>>) {
    match cmd {
        ...
        _ => {
            ...
            out.push(
                "  crawler rejections <domain> - Show rejections for a specific domain".to_string(),
            );
        }
    }
}
...
async fn show_rejection_stats(output_lines: &Arc<Mutex<Vec<String>>>) {
    match crate::services::crawler::CrawlRejected::get_stats().await {
        Ok(stats) => {
            let mut out = output_lines.lock().await;
            ...
            out.push(format!(
                "Rejections in last hour: {}",
                stats["rejections_last_hour"]
            ));
            out.push("".to_string());
...
async fn show_domain_rejections(domain: &str, output_lines: &Arc<Mutex<Vec<String>>>) {
    match crate::services::crawler::CrawlRejected::get_domain_rejections(domain).await {
        Ok(rejections) => {
            let mut out = output_lines.lock().await;
