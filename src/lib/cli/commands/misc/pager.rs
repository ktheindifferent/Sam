...
pub async fn handle_less(
    ...
    if args.len() < 2 {
        ...
        out.push("Commands: j/k (scroll), q (quit), g/G (go to start/end), /<search> (search)".to_string());
        return;
    }
    ...
    match std::fs::read_to_string(&file_path) {
        Ok(content) => {
            ...
            let visible_lines = if output_height > 3 { output_height - 3 } else { 1 };
            ...
            for (i, line_content) in lines.iter().enumerate().skip(start_line).take(visible_lines) {
...
pub async fn handle_less_nav(
    ...
    match cmd {
        "j" | "down" => {
            ...
            // Refresh display
            handle_less(&format!("less {}", filename), output_lines, current_dir, output_height, scroll_offset).await;
        }
        "k" | "up" => {
            // Scroll up one line
            if *scroll_offset > 0 {
        ...
        _ if cmd.starts_with('/') => {
            ...
            if !search_term.is_empty() {
                handle_less_search(filename, search_term, output_lines, current_dir, output_height, scroll_offset).await;
            }
        }
        _ => {
            let mut out = output_lines.lock().await;
            out.push(format!("Unknown less command: {}. Use j/k to scroll, g/G for start/end, q to quit.", cmd));
        }
    }
}
                ...
                if line.to_lowercase().contains(&search_term.to_lowercase()) {
                    *scroll_offset = i as u16;
                    ...
                    if current_len > 0 {
                        ...
                        out[current_len - 1] = format!("Found '{}' at line {} - j/k:scroll, n:next, q:quit", search_term, i + 1);
                    }
                    return;
                }
            }
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                if line.to_lowercase().contains(&search_term.to_lowercase()) {
                    *scroll_offset = i as u16;
                    ...
                    let current_len = out.len();
                    if current_len > 0 {
                        out[current_len - 1] = format!("Found '{}' at line {} (wrapped) - j/k:scroll, n:next, q:quit", search_term, i + 1);
                    }
                    return;
                }
            }
