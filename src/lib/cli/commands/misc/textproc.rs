...
pub async fn handle_sort(cmd: &str, output_lines: &Arc<Mutex<Vec<String>>>, current_dir: &PathBuf) {
    ...
    if numeric {
        lines_to_sort.sort_by(|a, b| {
            let a_num = extract_sort_key(a, field_separator, key_field).parse::<f64>().unwrap_or(0.0);
            let b_num = extract_sort_key(b, field_separator, key_field).parse::<f64>().unwrap_or(0.0);
            if reverse {
