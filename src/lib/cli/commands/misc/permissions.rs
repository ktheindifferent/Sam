...
pub async fn handle_chmod(cmd: &str, output_lines: &Arc<Mutex<Vec<String>>>, current_dir: &PathBuf) {
...
pub async fn handle_chown(cmd: &str, output_lines: &Arc<Mutex<Vec<String>>>, current_dir: &PathBuf) {
    let args: Vec<&str> = cmd.split_whitespace().collect();
    ...
    let (owner, group) = if owner_arg.contains(':') {
        ...
        (Some(parts[0]), if parts[1].is_empty() { None } else { Some(parts[1]) })
    } else {
        (Some(owner_arg), None)
    };
...
#[cfg(unix)]
fn chmod_file(path: &PathBuf, mode: u32, recursive: bool) -> Result<(), Box<dyn std::error::Error>> {
...
fn chmod_file(_path: &PathBuf, _mode: u32, _recursive: bool) -> Result<(), Box<dyn std::error::Error>> {
...
fn chown_file(path: &PathBuf, owner: Option<&str>, group: Option<&str>, recursive: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if owner.is_some() || group.is_some() {
        return Err("chown functionality requires elevated privileges and is not fully implemented".into());
    }
...
#[cfg(windows)]
fn chown_file(_path: &PathBuf, _owner: Option<&str>, _group: Option<&str>, _recursive: bool) -> Result<(), Box<dyn std::error::Error>> {
