    ...
    match subcommand {
        ...
        _ => {
            println!(
                "{}",
                format!(
                    "Unknown subcommand: {}. Use 'sam migrate help' for usage.",
                    subcommand
                )
                .red()
            );
    ...
    match perform_migrations(false, false).await {
        Ok(count) => {
            if count > 0 {
                println!(
                    "{}",
                    format!("✓ Successfully applied {} migration(s)", count).green()
                );
            } else {
    ...
    match perform_rollback(target_version).await {
        Ok(count) => {
            if count > 0 {
                println!(
                    "{}",
                    format!("✓ Successfully rolled back {} migration(s)", count).green()
                );
            } else {
...
async fn show_status() {
    ...
    match get_migration_status().await {
        ...
        Err(e) => {
            eprintln!(
                "{}",
                format!("✗ Failed to get migration status: {}", e).red()
            );
            std::process::exit(1);
        }
    }
...
impl super::Migration for Migration {{
    ...
    println!(
        "{}",
        format!(
            "Created migration template: src/sam/db/migrations/{}",
            filename
        )
        .green()
    );
    ...
    match check_migration_checksums().await {
        Ok(conflicts) => {
            if conflicts.is_empty() {
                ...
                println!(
                    "{}",
                    format!("⚠ Found {} checksum conflict(s):", conflicts.len()).red()
                );
...
fn show_help() {
    ...
    println!(
        "  {:<20} Rollback to previous migration",
        "down|rollback".green()
    );
    println!(
        "  {:<20} Rollback to specific version",
        "down|rollback <version>".green()
    );
    println!("  {:<20} Show migration status", "status".green());
    println!(
        "  {:<20} Create a new migration file",
        "create <name>".green()
    );
...
async fn perform_migrations(dry_run: bool, auto_backup: bool) -> Result<usize> {
    let pool = crate::services::pg::connect().await?;
...
async fn get_migration_status() -> Result<crate::db::migrations::MigrationStatus> {
    let pool = crate::services::pg::connect().await?;
    let migrations = crate::db::migrations::load_migrations();

    let runner = crate::db::migrations::MigrationRunner::new(pool).with_migrations(migrations);

    runner.status().await
}

async fn check_migration_checksums() -> Result<Vec<crate::db::migrations::MigrationConflict>> {
    let pool = crate::services::pg::connect().await?;
    let migrations = crate::db::migrations::load_migrations();
