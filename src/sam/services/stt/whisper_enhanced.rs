...
impl WhisperEngine {
    pub fn new(config: WhisperConfig) -> Result<Self, crate::sam::services::Error> {
        if !config.model_path.exists() {
            return Err(crate::sam::services::Error::from(format!(
        ...
        let ctx = WhisperContext::new_with_params(
            config.model_path.to_str().ok_or_else(|| {
                crate::sam::services::Error::from("Invalid model path")
            })?,
            params,
        )
        .map_err(|e| crate::sam::services::Error::from(format!("Failed to load model: {}", e)))?;
    ...
    pub fn transcribe(&self, audio_data: &[f32]) -> Result<WhisperResult, crate::sam::services::Error> {
        let start_time = std::time::Instant::now();
        
        let ctx = self.context.lock().map_err(|e| {
            crate::sam::services::Error::from(format!("Failed to lock context: {}", e))
        })?;
    ...
    pub fn transcribe_file(&self, audio_path: &Path) -> Result<WhisperResult, crate::sam::services::Error> {
    ...
    fn extract_results(&self, state: &mut whisper_rs::WhisperState) -> Result<(String, Vec<WhisperSegment>, String), crate::sam::services::Error> {
        let num_segments = state.full_n_segments().map_err(|e| {
            crate::sam::services::Error::from(format!("Failed to get segments: {}", e))
        })?;
        ...
        for i in 0..num_segments {
            let text = state.full_get_segment_text(i).map_err(|e| {
                crate::sam::services::Error::from(format!("Failed to get segment text: {}", e))
            })?;
            
            let start_time = state.full_get_segment_t0(i).map_err(|e| {
                crate::sam::services::Error::from(format!("Failed to get segment start time: {}", e))
            })?;
    ...
    fn load_audio_file(&self, path: &Path) -> Result<Vec<f32>, crate::sam::services::Error> {
        use hound::WavReader;
        
        let mut reader = WavReader::open(path).map_err(|e| {
            crate::sam::services::Error::from(format!("Failed to open audio file: {}", e))
        })?;
        
        let spec = reader.spec();
        
        if spec.channels != 1 {
            return Err(crate::sam::services::Error::from(
        ...
        samples.map_err(|e| {
            crate::sam::services::Error::from(format!("Failed to read audio samples: {}", e))
        })
    }
    ...
    pub fn convert_audio_to_16khz_mono(
        ...
        let status = Command::new("ffmpeg")
            ...
            .status()
            .map_err(|e| {
                crate::sam::services::Error::from(format!("Failed to run ffmpeg: {}", e))
            })?;
...
impl WhisperService {
    pub fn new() -> Result<Self, crate::sam::services::Error> {
    ...
    pub fn with_config(config: WhisperConfig) -> Result<Self, crate::sam::services::Error> {
    ...
    pub fn transcribe(&self, audio_data: &[f32]) -> Result<WhisperResult, crate::sam::services::Error> {
        let engines = self.engines.lock().map_err(|e| {
            crate::sam::services::Error::from(format!("Failed to lock engines: {}", e))
        })?;
        
        if engines.is_empty() {
            return Err(crate::sam::services::Error::from("No engines available"));
        }
    ...
    pub fn add_model(&mut self, config: WhisperConfig) -> Result<(), crate::sam::services::Error> {
