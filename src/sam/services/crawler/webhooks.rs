...
impl WebhookSender {
    ...
    pub async fn send(&self, event: WebhookEvent, job_id: String, job_url: String, data: serde_json::Value, metadata: WebhookMetadata) -> Result<()> {
        // Check if this event should be sent
        if !self.config.events.contains(&event) {
            debug!("Skipping webhook for event {:?} (not in configured events)", event);
            return Ok(());
        }
        ...
        let payload = WebhookPayload {
            ...
            job_id: job_id.clone(),
            job_url,
            data: if self.config.include_full_data { data } else { json!({}) },
            metadata,
        };
        ...
        for attempt in 0..=self.config.retry_count {
            if attempt > 0 {
                info!("Retrying webhook (attempt {}/{})", attempt, self.config.retry_count);
...
impl WebhookNotificationService {
    ...
    pub async fn notify_job_completed(&self, job_id: String, job_url: String, metadata: WebhookMetadata) {
        let data = json!({
            "status": "completed",
    ...
    pub async fn notify_job_failed(&self, job_id: String, job_url: String, error: String, metadata: WebhookMetadata) {
        let data = json!({
            "status": "failed",
            "error": error,
    ...
    pub async fn notify_milestone(&self, job_id: String, job_url: String, milestone: String, metadata: WebhookMetadata) {
        let data = json!({
            "milestone": milestone,
