...
impl FileStorageService {
    ...
    pub async fn store_file(
        ...
        // Compress if needed
        let (final_data, is_compressed, compression_ratio) = if self.should_compress(&data, filename) {
    ...
    pub async fn get_file(&self, file_id: &str, user: &str) -> Result<(Vec<u8>, FileMetadata), StorageError> {
        // Get metadata
        ...
        // Decrypt if needed
        if matches!(metadata.encryption_status, EncryptionStatus::Encrypted { .. }) {
    ...
    pub async fn create_version(
        ...
        let (final_data, is_compressed, compression_ratio) = if self.should_compress(&data, &metadata.filename) {
            let compressed = self.compress_data(&data).await?;
            let ratio = compressed.len() as f32 / data.len() as f32;
            (compressed, true, Some(ratio))
        } else {
        ...
        if metadata.version > self.config.max_versions {
            self.cleanup_old_versions(&metadata.id, metadata.version - self.config.max_versions).await?;
        }
    ...
    async fn generate_thumbnails(&self, data: &[u8], file_id: &str) -> Result<PathBuf, StorageError> {
    ...
    fn has_read_permission(&self, metadata: &FileMetadata, user: &str) -> bool {
        metadata.permissions.owner == user || 
        metadata.permissions.read.contains(&user.to_string())
    }
    
    fn has_write_permission(&self, metadata: &FileMetadata, user: &str) -> bool {
        metadata.permissions.owner == user || 
        metadata.permissions.write.contains(&user.to_string())
    }
    ...
    async fn cleanup_old_versions(&self, file_id: &str, versions_to_delete: u32) -> Result<(), StorageError> {
