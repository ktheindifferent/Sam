...
impl BackupService {
    ...
    pub async fn execute_full_backup(&self) -> Result<BackupMetadata> {
        ...
        if *self.is_running.read().await {
            return Err(ServiceError::ServiceUnavailable("Backup already in progress".to_string()).into());
        }
        *self.is_running.write().await = true;
        ...
        for target in &self.config.targets {
            ...
            // Update metrics
            self.metrics.increment_counter("backup_targets_processed", HashMap::new()).await;
        }
        ...
        *self.is_running.write().await = false;
        ...
        self.metrics.increment_counter("backups_completed", HashMap::new()).await;
    ...
    pub async fn execute_incremental_backup(&self, parent_id: &str) -> Result<BackupMetadata> {
        ...
        // Get parent metadata
        let parent_metadata = self.get_metadata(parent_id).await
    ...
    pub async fn restore_backup(&self, backup_id: &str, restore_path: &Path) -> Result<()> {
        ...
        let metadata = self.get_metadata(backup_id).await
        ...
        for target_info in &metadata.targets {
            ...
            self.restore_target(&target_backup_path, &target_restore_path).await?;
            
            info!("Restored target: {} to {:?}", target_info.name, target_restore_path);
        }
        
        self.metrics.increment_counter("restores_completed", HashMap::new()).await;
    ...
    pub async fn verify_backup(&self, metadata: &BackupMetadata) -> Result<()> {
        ...
        if calculated_checksum != metadata.checksum {
            return Err(anyhow::anyhow!("Backup verification failed: checksum mismatch"));
        }
        ...
        for target in &metadata.targets {
            ...
            if target_checksum != target.checksum {
                return Err(anyhow::anyhow!("Target verification failed: {}", target.name));
            }
        }
    ...
    pub async fn apply_retention_policy(&self) -> Result<()> {
        ...
        for backup in backups {
            ...
            if age < ChronoDuration::days(1) || daily_count < self.config.retention.daily_backups {
                daily_count += 1;
            } else if age < ChronoDuration::weeks(1) || weekly_count < self.config.retention.weekly_backups {
                weekly_count += 1;
            } else if age < ChronoDuration::days(30) || monthly_count < self.config.retention.monthly_backups {
                monthly_count += 1;
        ...
        for backup_id in to_delete {
            if let Some(metadata) = metadata_store.remove(&backup_id) {
                ...
                if let Err(e) = fs::remove_dir_all(&backup_path).await {
                    ...
                    self.metrics.increment_counter("backups_deleted", HashMap::new()).await;
                }
            }
        }
    ...
    async fn backup_target(&self, target: &BackupTarget, backup_dir: &Path) -> Result<BackupTargetInfo> {
        ...
        for include_path in &target.include_paths {
            if include_path.is_file() {
                let dest = target_backup_dir.join(include_path.file_name().unwrap());
                ...
                self.backup_directory(include_path, &target_backup_dir, &mut total_size, &mut file_count, &mut hasher).await?;
            }
        }
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if path.is_file() {
                ...
                *file_count += 1;
                ...
                Box::pin(self.backup_directory(&path, &dest_path, total_size, file_count, hasher)).await?;
            }
        }
    ...
    async fn backup_changed_files(
        ...
        if source.is_file() {
            ...
            while let Some(entry) = entries.next_entry().await? {
                ...
                Box::pin(self.backup_changed_files(&path, &dest_path, since, total_size, file_count, hasher)).await?;
            }
        }
    ...
    async fn store_metadata(&self, metadata: &BackupMetadata) -> Result<()> {
        self.metadata_store.write().await.insert(metadata.id.clone(), metadata.clone());
        
        // Also persist to disk
        let metadata_file = self.config.base_path.join("metadata.json");
        let all_metadata: Vec<BackupMetadata> = self.metadata_store.read().await.values().cloned().collect();
...
impl HealthCheckable for BackupService {
    async fn check(&self) -> Result<HealthCheck> {
        ...
        if let Err(e) = fs::metadata(&self.config.base_path).await {
            health.status = HealthStatus::Unhealthy(format!("Backup directory inaccessible: {}", e));
            return Ok(health);
        }
        ...
        // Add metadata
        health.metadata.insert("backup_count".to_string(), self.metadata_store.read().await.len().to_string());
        health.metadata.insert("is_running".to_string(), self.is_running.read().await.to_string());
        health.metadata.insert("available_space_gb".to_string(), (available_space / (1024 * 1024 * 1024)).to_string());
...
mod tests {
    ...
    async fn test_backup_and_restore() {
        ...
        // Execute backup
        let metadata = service.execute_full_backup().await.unwrap();
        assert_eq!(metadata.backup_type, BackupType::Full);
        
        // Verify backup
        service.verify_backup(&metadata).await.unwrap();
        
        // Restore backup
        service.restore_backup(&metadata.id, restore_dir.path()).await.unwrap();
        
        // Verify restored file
        let restored_file = restore_dir.path().join("test_target").join("test.txt");
        assert!(restored_file.exists());
        
        let content = fs::read_to_string(restored_file).await.unwrap();
