...
impl TtsService {
    pub fn new(config: TtsConfig) -> Result<Self, crate::sam::services::Error> {
        if config.cache_enabled {
            fs::create_dir_all(&config.cache_dir).map_err(|e| {
                crate::sam::services::Error::from(format!("Failed to create cache dir: {}", e))
            })?;
        }
    ...
    pub fn synthesize(&self, request: TtsRequest) -> Result<TtsResult, crate::sam::services::Error> {
    ...
    fn synthesize_system(&self, request: &TtsRequest) -> Result<Vec<u8>, crate::sam::services::Error> {
    ...
    #[cfg(target_os = "windows")]
    fn synthesize_windows(&self, request: &TtsRequest) -> Result<Vec<u8>, crate::sam::services::Error> {
        ...
        let output = Command::new("powershell")
            .args(&["-Command", &script])
            .output()
            .map_err(|e| crate::sam::services::Error::from(format!("PowerShell error: {}", e)))?;
        
        if !output.status.success() {
            return Err(crate::sam::services::Error::from("Windows TTS failed"));
        }
        
        let mut file = File::open(&tmp_path).map_err(|e| {
            crate::sam::services::Error::from(format!("Failed to read TTS output: {}", e))
        })?;
        let mut data = Vec::new();
    ...
    #[cfg(target_os = "macos")]
    fn synthesize_macos(&self, request: &TtsRequest) -> Result<Vec<u8>, crate::sam::services::Error> {
        use rand::{distributions::Alphanumeric, Rng};
        use std::process::Command;
        ...
        let output = Command::new("say")
            .args(&args)
            .output()
            .map_err(|e| crate::sam::services::Error::from(format!("macOS say error: {}", e)))?;
        
        if !output.status.success() {
            return Err(crate::sam::services::Error::from("macOS TTS failed"));
        }
    ...
    #[cfg(target_os = "linux")]
    fn synthesize_linux(&self, request: &TtsRequest) -> Result<Vec<u8>, crate::sam::services::Error> {
        use rand::{distributions::Alphanumeric, Rng};
        use std::process::Command;
        ...
        Err(crate::sam::services::Error::from(
            "No TTS engine available on Linux (espeak or pico2wave required)"
        ))
    }

    fn synthesize_mozilla(&self, request: &TtsRequest) -> Result<Vec<u8>, crate::sam::services::Error> {
        ...
        let response = client
            ...
            .send()
            .map_err(|e| crate::sam::services::Error::from(format!("Mozilla TTS error: {}", e)))?;
        
        if !response.status().is_success() {
            return Err(crate::sam::services::Error::from("Mozilla TTS request failed"));
        }
    ...
    fn synthesize_external(&self, request: &TtsRequest, url: &str) -> Result<Vec<u8>, crate::sam::services::Error> {
        let client = reqwest::blocking::Client::builder()
            .timeout(self.config.timeout)
            .build()?;
        ...
        let response = client
            ...
            .json(&params)
            .send()
            .map_err(|e| crate::sam::services::Error::from(format!("External TTS error: {}", e)))?;
        
        if !response.status().is_success() {
            return Err(crate::sam::services::Error::from("External TTS request failed"));
        }
    ...
    fn synthesize_coqui(&self, request: &TtsRequest) -> Result<Vec<u8>, crate::sam::services::Error> {
        Err(crate::sam::services::Error::from(
            "Coqui TTS integration not yet implemented"
        ))
    }

    pub fn list_voices(&self) -> Result<Vec<String>, crate::sam::services::Error> {
