...
pub fn handle(
    ...
    if request.url() == "/api/services/notifications/unseen" && request.method() == "GET" {
        ...
        pg_query
            .queries
            .push(crate::sam::memory::PGCol::Boolean(false));
        pg_query.query_columns.push("seen =".to_string());

        pg_query
            .queries
            .push(crate::sam::memory::PGCol::String(current_session.human_oid));
        pg_query.query_columns.push(" AND human_oid =".to_string());

        pg_query
            .queries
            .push(crate::sam::memory::PGCol::String(current_session.sid));
        pg_query.query_columns.push(" AND sid =".to_string());

        let notifications = crate::sam::memory::human::Notification::select(
            None,
            None,
    ...
    if request.url() == "/api/services/notifications/seen" {
        ...
        pg_query
            .queries
            .push(crate::sam::memory::PGCol::String(input.oid.clone()));
