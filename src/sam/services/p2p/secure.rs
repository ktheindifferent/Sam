...
impl SecureP2P {
    ...
    fn create_handshake_message(&self, ephemeral_public_key: Vec<u8>) -> Result<HandshakeMessage, Box<dyn std::error::Error>> {
    ...
    pub async fn encrypt_message(&self, peer_id: &str, message: &[u8]) -> Result<EncryptedMessage, Box<dyn std::error::Error>> {
        // Check message size
        ...
        let channel = channels.get(&PeerId(peer_id.to_string()))
    ...
    pub async fn decrypt_message(&self, encrypted_msg: &EncryptedMessage) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        ...
        let channels = self.secure_channels.read().await;
        let channel = channels.get(&encrypted_msg.sender_id)
        ...
        public_key.verify(&data_to_verify, &encrypted_msg.signature)
    ...
    async fn rotate_session_key(&self, channel: &SecureChannel) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn get_trust_level(&self, peer_id: &str) -> TrustLevel {
        ...
        trust.get(&PeerId(peer_id.to_string()))
            .copied()
...
mod tests {
    ...
    async fn test_secure_p2p_creation() {
        ...
        assert_eq!(secure_p2p.get_trust_level("unknown").await, TrustLevel::Untrusted);
    }
