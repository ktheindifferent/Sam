...
use crate::sam::resource_management::{
    ResourceManager, ResourceConfig, FileLimits, RequestLimits, MemoryLimits,
    PoolConfig, CleanupConfig
};
use crate::sam::http::rate_limiter::{RateLimiter, RateLimitConfig};
...
async fn check_upload_limits(request: &Request) -> Option<Response> {
    ...
    match RESOURCE_MANAGER.check_upload_allowed(&user_id, file_size, &file_extension).await {
        Ok(permission) => {
            match permission {
                crate::sam::resource_management::UploadPermission::Allowed { .. } => None,
                crate::sam::resource_management::UploadPermission::Denied { reason } => {
...
pub async fn get_metrics() -> crate::sam::resource_management::ResourceMetrics {
