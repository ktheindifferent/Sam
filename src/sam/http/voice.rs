...
use crate::sam::services::voice::{VoiceService, VoiceConfig};
use crate::sam::services::stt::whisper_enhanced::{WhisperConfig, WhisperService};
use crate::sam::services::tts::{TtsConfig, TtsService, TtsRequest, AudioFormat};
...
fn handle_transcribe(request: &Request) -> Result<Response, crate::sam::http::Error> {
    ...
    let timestamp = SystemTime::now()
        ...
        .map_err(|e| crate::sam::http::Error::from(format!("Time error: {}", e)))?
        .as_secs();
    ...
    let service = WhisperService::with_config(config)
        .map_err(|e| crate::sam::http::Error::from(format!("Whisper init error: {}", e)))?;
    
    let result = service.transcribe_file(&tmp_file_path)
        .map_err(|e| crate::sam::http::Error::from(format!("Transcription error: {}", e)))?;
...
fn handle_synthesize(request: &Request) -> Result<Response, crate::sam::http::Error> {
    ...
    let service = TtsService::new(TtsConfig::default())
        .map_err(|e| crate::sam::http::Error::from(format!("TTS init error: {}", e)))?;
    
    let result = service.synthesize(tts_request)
        .map_err(|e| crate::sam::http::Error::from(format!("Synthesis error: {}", e)))?;
...
fn handle_voice_command(request: &Request) -> Result<Response, crate::sam::http::Error> {
...
fn handle_config(request: &Request) -> Result<Response, crate::sam::http::Error> {
    if request.method() == "GET" {
        ...
        VOICE_SERVICE.initialize(config.clone())
            .map_err(|e| crate::sam::http::Error::from(format!("Config error: {}", e)))?;
...
fn handle_status(request: &Request) -> Result<Response, crate::sam::http::Error> {
    #[derive(Serialize)]
